...
clear;clear;clear
  ...
  # // Banner
echo -e "${YELLOW}----------------------------------------------------------${NC}"
echo -e "  Welcome To RMBL VPN Script Installer ${YELLOW}(${NC}${green} Stable Edition ${NC}${YELLOW})${NC}"
echo -e "     This Will Quick Setup VPN Server On Your Server"
echo -e "         Auther : ${BLUE}RMBL  ${NC}${YELLOW}(${NC} ${BLUE}VPN ${NC}${YELLOW})${NC}"
echo -e "${YELLOW}----------------------------------------------------------${NC}"
echo ""
sleep 5
###### IZIN SC
...
if [[ $IP == "" ]]; then
    ...
    echo -e "${OK} IP Address ( ${BLUE}$IP${NC} )"
fi
...
expx=$(curl https://raw.githubusercontent.com/RMBL-ZERO/permission/main/ipmini | grep $MYIP | awk '{print $3}')
echo "$expx" >/usr/bin/e
# DETAIL ORDER
...
# Status ExpiRED Active | RMBL VPN Project
...
clear
# REPO
    REPO="https://raw.githubusercontent.com/RMBL-ZERO/rmbl/main/"
...
clear
    ...
    restart_system() {
    USRSC=$(wget -qO- https://raw.githubusercontent.com/RMBL-ZERO/permission/main/ipmini | grep $ipsaya | awk '{print $2}')
    EXPSC=$(wget -qO- https://raw.githubusercontent.com/RMBL-ZERO/permission/main/ipmini | grep $ipsaya | awk '{print $3}')
...
print_install "Memasang modul SlowDNS Server"
    wget -q -O /tmp/nameserver "${REPO}sanakstore/nameserver" >/dev/null 2>&1
